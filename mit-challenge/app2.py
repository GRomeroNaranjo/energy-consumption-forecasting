...
for i in range(len(data_series) - num - 19):
    data_x.append(data_series[i : i + num])
    data_y.append(data_series[i + num : i + num + 20])
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.linear = nn.Linear(config.mlp2_output_size, 20)
...
class System():
    ...
    def fit_scalers(self, X_train, y_train):
        ...
        y_train_flattened = np.array(y_train).reshape(-1, 1)
        self.y_scaler.fit(y_train_flattened)
    ...
    def train(self, epochs, X_train, y_train, batch_size=64):
        ...
        y_train_scaled = [self.scale_data(y, self.y_scaler, 1) for y in y_train]
        y_train_tensor = np.array(y_train_scaled)
    ...
    def predict_long(self, x, num):
        ...
        for _ in range(num // 20):
            ...
            if isinstance(y, np.ndarray) and y.ndim == 2:
                prediction_list.extend(y[0])
                x = np.roll(x, -20)
                x[-20:] = y[0]
            elif isinstance(y, np.ndarray) and y.ndim == 1 and len(y) == 20:
                prediction_list.extend(y)
                x = np.roll(x, -20)
                x[-20:] = y
            else:
...
test_accuracy_whole_predictions(X_train, y_train, X_test, y_test, config, 1, len(y_test))
